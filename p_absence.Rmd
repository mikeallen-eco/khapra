...
```{r}
### 0. Beta(0.25, 4)
pp0 = dbeta(seq(0,1,0.01), 0.5, 4) %>% 
  tibble() %>% mutate(x = seq(0,1,0.01)) %>% 
  ggplot(aes(x=x, y = .)) +
  geom_line() + 
  scale_x_continuous(limits = c(0,1), breaks = scales::pretty_breaks(5)) + 
  labs(x = "probability", y = "density", subtitle = "Beta(0.5, 4)")
# pp0

### 1. Uniform(0.01, 0.1)
pp1 = dunif(seq(0,1,0.01), 0.001, 0.2) %>% 
  tibble() %>% mutate(x = seq(0,1,0.01)) %>% 
  ggplot(aes(x=x, y=.)) +
  # geom_density(adjust = 2) + 
  geom_line() + 
  scale_x_continuous(limits = c(0,1), breaks = scales::pretty_breaks(5)) +
  labs(x = "probability", y = "density", subtitle = "Uniform(0.001, 0.20)")
# pp1
...
sa2 = p_abs_multi(i, runif(i, 0.001, 0.2)) %>% 
  p_abs_sum() %>% 
  mutate(Pprior = "Uniform(0.001, 0.20)")
...
sa_min = 
  ...
  mutate(Pprior = 
           forcats::fct_relevel(Pprior, 
                   c("Beta(0.5, 4)", "Uniform(0.001, 0.20)", 
                     "Beta(1, 5)", "Beta(1, 3)")
                     )) %>% 
