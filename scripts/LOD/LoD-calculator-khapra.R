...
setwd("/Users/mikea/Documents/research/khapra/scripts/LOD/")

## Read in your data file (MODIFY FILE NAME AS NEEDED):
  DAT <- read.csv("khapra_LOD.csv") %>%
    filter(Task == "STANDARD") %>%
    rename(Cq = 7) %>%
    mutate(order = 1:93,
           run_ID = "A",
           logSQ = log(Quantity),
           Cq = as.numeric(case_when(Cq == "Undetermined" ~ as.numeric(NA),
                          TRUE ~ as.numeric(Cq)))) %>%
    dplyr::select(order, run_ID, Well, Reporter, Quencher, 
           Target = 'Target Name', SQ = Quantity,
           logSQ, Cq)
  # order	run_ID	Well	Reporter	Quencher	Target	SQ	logSQ	Cq
...
LOD.FCT <- LL.3u()
# worked for LOD: LL2.3, LL2.3u, LL2.4, LL2.5, LL.4, LL.3u, LL.3, MM.3, & MM.2; LL.3u had best fit according to p-value

# none of the others had "solvable" curves
#W1.2, W1.3, W1.4, W2.3, W2.4, BC.4, BC.5, 
# LL2.3, LL2.3u, LL2.4, LL2.5, AR.2, AR.3, LL.4 LL.3u LL.3 LL.2() 
#"Best" MM.3, MM.2 
...
for(i in 1:length(Targets)) {
  if(!is.na(LOD.list2[i+1])) {
    ...
    plot(get(LOD.list2[i+1]),main=paste0("LoD Plot for: ",Targets[i]),
         ylab="Detection Probability",xlab="Quantity of DNA",
    ...
    legend("bottomright",legend=LODS2,pch=PNTS,col=COLS,text.col=COLS, cex = 0.5)
